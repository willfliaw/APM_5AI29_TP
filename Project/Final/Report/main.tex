...
\todo{further explain KGC Retriever}
...
For our experiments, we employed two widely used benchmarks for KGC, with their statistics provided in \cref{tab:datasets}.

\textbf{WN18RR} is a refined version of the WN18 dataset \cite{wn18rr}, derived from WordNet \cite{wordnet}. It was introduced to address data leakage issues in WN18 by removing inverse relations that made link prediction tasks trivial. Due to the removal of these redundant relations, WN18RR presents a more challenging benchmark for knowledge graph completion, requiring models to generalize beyond simple pattern memorization.
