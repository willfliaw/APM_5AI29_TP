...
% Slide 2: Prompt-Based vs Fine-Tuning-Based KGC
\begin{frame}{Background - Early KGC to LLM-Based Approaches}
\vspace{0.4cm}
\begin{center}
\textbf{\textit{Why Embedding to LLM ?}}
\end{center}
\vspace{0.2cm}
\begin{center}
    \begin{tikzpicture}
        \node[draw, rectangle, fill=blue!20, minimum width=3cm] (early) at (0,0) {Early KGC};
        \node[draw, rectangle, fill=green!20, minimum width=3cm] (llm) at (7,0) {LLM-Based KGC};
        \draw[->, thick] (early) -- node[above] {\tiny Better Adaptability, No Retraining} (llm);
    \end{tikzpicture}
\end{center}
\end{frame}
